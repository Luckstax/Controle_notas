...
Alocacoes, Saldo por NF, Avisos, Controle, Retorno por NF e Fornecedor
e Resumo por Fornecedor.
"""
...
# ----------------------------------------------------------------------
# Aba "Resumo por Fornecedor": uma linha por (Fornecedor, Produto) com
# o total enviado e o total retornado, somando TUDO daquele fornecedor
# e produto de uma vez - sem olhar qual NF especifica cada retorno
# pertence. E' a visao "macro" que complementa o Saldo por NF (que e'
# por NF individual): serve pra bater o olho rapido se sobrou/faltou
# nota de saida lancada para aquele fornecedor/produto (Diferenca
# negativa = ja voltou mais do que foi enviado segundo o que esta
# lancado - sinal de nota de saida faltando ou lancada errada).
# ----------------------------------------------------------------------
@com_retry
def escrever_resumo_fornecedor(sh, saidas, entradas):
    total_saida = {}
    total_entrada = {}
    for s in saidas:
        for produto, qtd in s.produtos.items():
            chave = (s.fornecedor, produto)
            total_saida[chave] = total_saida.get(chave, 0) + qtd
    for e in entradas:
        for produto, qtd in e.produtos.items():
            chave = (e.fornecedor, produto)
            total_entrada[chave] = total_entrada.get(chave, 0) + qtd

    chaves = sorted(set(total_saida) | set(total_entrada))
    if not chaves:
        return

    cabecalho = ["Fornecedor", "Produto", "Total Enviado", "Total Retornado", "Diferenca"]
    linhas = [cabecalho]
    for fornecedor, produto in chaves:
        enviado = round(total_saida.get((fornecedor, produto), 0), 6)
        retornado = round(total_entrada.get((fornecedor, produto), 0), 6)
        diferenca = round(enviado - retornado, 6)
        linhas.append([fornecedor, produto, enviado, retornado, diferenca])

    ws = obter_ou_criar_aba(sh, "Resumo por Fornecedor")
    ws.update(values=linhas, range_name="A1")
    ws.format("A1:E1", {"textFormat": {"bold": True}, "backgroundColor": COR_CINZA})
    ws.freeze(rows=1)
    print("Aba 'Resumo por Fornecedor' atualizada.")
    return ws
